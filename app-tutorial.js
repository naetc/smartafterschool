...
window.TUTORIAL_CONTENT = {
    basic: [
        ...
        {
            title: '🎯 실수해도 괜찮아요 — 되돌리기 버튼',
            highlight: '#btnUndo',
            body: `화면 상단의 <strong>↩️ 되돌리기</strong> 버튼은 바로 직전에 한 작업 <strong>딱 1건만</strong>
                   취소해줘요. 누르면 되돌릴 작업이 무엇인지 먼저 보여주고 확인을 받기 때문에, 실수로
                   잘못 눌러서 원치 않는 게 되돌아갈 걱정은 안 해도 돼요.<br><br>
                   다만 스택이 아니라 <strong>단 1건만</strong> 기억하는 구조라서, 되돌리기 대상이 있는
                   상태에서 다른 작업을 하나 더 하면 이전 되돌리기 대상은 사라지고 방금 한 작업으로
                   교체돼요. 그리고 <strong>차수 마감 처리는 되돌리기 대상에서 제외</strong>되어 있어서,
                   마감을 되돌리기로 실수로 푸는 일은 없습니다.`
        },
        {
            ...
            body: `여기서 입력하는 <strong>월 강사료 / 월 수용비</strong>는 "한 달(4주) 기준" 금액이고,
                   ...
                   나타내요. 실제 진행 시수가 기준보다 많으면 그만큼 더 청구되고, 적으면 덜 청구됩니다.<br><br>
                   💡 나눗셈이 딱 떨어지지 않을 때는 <strong>강사료는 원 단위 내림, 수용비는 원 단위
                   올림</strong>으로 계산해요. 강사에게 계약 비율보다 더 지급되는 일이 없게, 반대로
                   운영비(수용비)는 부족해지지 않게 하기 위한 규칙이에요.<br><br>
                   ⚠️ 행정 규정상 <strong>월 수용비는 월 강사료의 5%를 넘을 수 없어요.</strong> 입력칸
                   바로 아래에 지금 비율과 5% 한도액이 실시간으로 표시되고, 초과하면 빨간 글씨로
                   경고가 뜨지만 입력 자체를 막지는 않으니 초과 표시가 보이면 직접 확인해서 고쳐주세요.`
        },
        {
        ...
        },
        {
            ...
            body: `강좌 요금표의 <strong>"운영"</strong> 체크박스는 그 분기에 이 부서(또는 개별 강좌)를
                   ...
                   자동 이동되어, 다른 강좌로 재배정해줘야 하는 상태가 됩니다.<br><br>
                   ⚠️ 이건 부서 목록의 <strong>"삭제"</strong> 버튼과는 완전히 다른 기능이에요.
                   "운영" 체크 해제는 이번 분기만 폐강 처리하는 것이지만, <strong>"삭제"는 1~4분기
                   전체의 마스터 데이터를 영구히 지우는 동작</strong>입니다. 그래서 이미 마감 처리된
                   분기 자료가 있는 부서는 삭제 자체가 막혀 있어요 — 그런 부서를 폐강하고 싶다면
                   삭제 대신 "운영" 체크를 해제하세요.`
        },
        {
        ...
        {
            title: '🎯 3스텝 — 자유수강권 관리 ②: 지원시점(언제부터 지원받는지)',
            tab: '#step3',
            body: `자유수강권 대상으로 등록할 때 <strong>"지원 시점"</strong>(지원이 시작되는 분기·차수)을
                   함께 정할 수 있어요. 기본값은 그 강좌를 처음 수강한 분기지만, 학기 중간에 지원
                   자격이 생긴 학생이라면 명단의 <strong>"지원 시점 수동 조작"</strong> 버튼으로
                   강좌별로 다르게 지정할 수 있습니다.<br><br>
                   지정한 시점 <strong>이전 구간은 자유수강권이 차감되지 않고 그대로 자부담</strong>으로
                   남아요. 다만 그 시작 시점이 분기 <strong>첫 차수 1시수째</strong>(교재비가 부과되는
                   시점)와 같거나 더 이르면 교재비 · 재료비도 함께 공제 대상이 되고, 그보다 늦게(도중에)
                   시작하면 교재비 · 재료비는 자부담으로 남습니다.<br><br>
                   ⚠️ 같은 학생을 자유수강권 명단에 <strong>중복으로 다시 등록하면 지원시점 설정이
                   조용히 무시</strong>될 수 있어요. 이미 등록된 학생은 새로 등록하지 말고 기존 항목의
                   "지원 시점 수동 조작"을 이용하세요. 옛날에 실수로 중복 등록된 학생이 있는지는 3스텝
                   상단의 <strong>"중복 등록 검사"</strong> 버튼으로 한 번에 찾아 정리할 수 있어요.`
        },
        {
        ...
        },
        {
            ...
            body: `<strong>학생 통합 콘솔</strong>: 학생별 탭에서 이름을 클릭하면 열려요 (이 두 콘솔은 4스텝뿐
                   ...
                   증감시켜요 (예: 다자녀 할인).<br>
                   💸 <strong>환불 처리</strong>: 개시전 · 일할계산 · 포기 사유별로 환불을 계산해요.
        ...
        },
        {
            title: '🎯 5스텝 — 서식 자동생성',
            tab: '#step5',
            body: `<strong>에듀파인 수납요구서, 교육비 청구서(업체), 환불 이력서, 증빙 명단(엑셀),
                   스마트출석부</strong> 5가지 서식을 만드는 단계입니다. 각 서식이 어떤 용도인지는
                   5스텝 화면의 안내를 참고하세요.`
        }
    ],
    advanced: [
        ...
        },
        {
            ...
            body: `이 시스템의 핵심 규칙은 <strong>"초3지원금을 먼저 다 쓰고, 그 다음에 자유수강권을 쓴다"</strong>는
                   ...
                   순서가 바뀌지 않습니다. (단 하나의 예외가 있는데, 다음 항목 "육아기 근로시간 단축"에서
                   다뤄요.)<br><br>
        ...
        {
            title: '🎯 육아기 근로시간 단축 — 우선순위가 뒤집히는 유일한 예외',
            tab: '#step3', highlight: '[onclick="window.openTransferModal()"]',
            body: `"초3지원금을 먼저 다 쓰고, 그 다음에 자유수강권을 쓴다"는 규칙에는 <strong>딱 하나의
                   예외</strong>가 있어요. 자유수강권 등록 시 구분(사유)을 <strong>"육아기 근로시간
                   단축"</strong>으로 지정하면, 그 학생이 초3 대상이더라도 <strong>자유수강권을 초3
                   지원금보다 먼저</strong> 소진합니다.<br><br>
                   이 구분을 선택하면 지원이 끝나는 <strong>종료 시점(분기 · 차수 · 시수)</strong>도 함께
                   지정할 수 있고, 종료 시점 이후는 자동으로 자유수강권 대상에서 제외되어 다시 원래
                   순서(초3 먼저)로 돌아가요. 명단에는 <strong>"육아"</strong> 뱃지가 붙어서 일반
                   자유수강권 대상자와 한눈에 구분됩니다.`,
            checklist: [
                '3스텝 → 자유수강권 관리에서 대상 학생의 구분(사유)을 "육아기 근로시간 단축"으로 지정해보기',
                '지원 종료 시점(분기·차수·시수)까지 입력하고 저장하기',
                '4스텝에서 이 학생의 산출근거를 열어, 자유수강권이 초3보다 먼저 소진됐는지 확인하기'
            ]
        },
        {
