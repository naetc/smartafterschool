...
// 크로스 라우팅 (헤더 뱃지 클릭 시 탭 이동 및 필터 켜기)
...
// 메인 렌더링 함수 (자유/초3 명단 동시 렌더링 및 헤더 업데이트)
window.renderF = function() {
    ...
    if (lsF.length === 0) {
        ...
        freeHtml = `<thead class="table-light"><tr><th>학적</th><th>이름</th><th>연간 총지원금</th><th class="table-success">현재 조정된 잔액</th><th class="table-warning">지원 시점 수동 조작</th><th>관리</th></tr></thead><tbody>` + lsF.map(f => {
            ...
            let nmBadge = f._hasTrans ? `<span class="badge bg-danger ms-1" style="font-size:0.7rem;">전입조정</span>` : '';
    ...
    if (lsC.length === 0) {
        ...
        cho3Html = `<thead class="table-light"><tr><th>학적</th><th>이름</th><th>연간 총지원금</th><th class="table-primary">현재 조정된 잔액</th><th class="table-secondary">상태</th></tr></thead><tbody>` + lsC.map(c => {
            ...
            const nmBadge = c._hasTrans ? `<span class="badge bg-danger ms-1" style="font-size:0.7rem;">전입조정</span>` : '';
    ...
    if(window.$('glbBadgeFree')) {
        ...
        window.$('glbBadgeFree').className = fTransCnt > 0 ? 'badge bg-danger text-white clickable p-2' : 'badge bg-success bg-opacity-10 border border-success text-success clickable p-2';
    }
    if(window.$('glbBadgeCho3')) {
        window.$('glbBadgeCho3').innerText = `초3지원: ${cTransCnt}명`;
        window.$('glbBadgeCho3').className = cTransCnt > 0 ? 'badge bg-danger text-white clickable p-2' : 'badge bg-primary bg-opacity-10 border border-primary text-primary clickable p-2';
    }
};
